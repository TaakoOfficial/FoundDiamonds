				...
				if (!strLine.startsWith("#")) {
					try {
						...
						for (int i = 0; i < oldmats.length; i++) {
							oldmats[i] = Material.matchMaterial(fs[2 + i]);
						}
						...
						Trap lo = new Trap(Byte.parseByte(fs[0]), Material.matchMaterial(fs[1]), oldmats, fs[fs.length - 7], temp, Long.parseLong(fs[fs.length - 2]), Boolean.parseBoolean(fs[fs.length - 1]));
					} catch (Exception ex) {
						fd.getLog().severe(MessageFormat.format("Invalid block in file.  Please delete {0}", file.getName()));
					}
				}
