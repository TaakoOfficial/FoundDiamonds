...
public class LoggingHandler {
	...
	public void handleLogging(Player player, Block block, boolean trapBlock, boolean kicked, boolean banned, boolean command) {
		try {
			...
			pw.println(" " + block.getType().name().toLowerCase().replace("_", " ") + " broken by " + player.getName() + " at (x: " + block.getX() + ", y: " + block.getY() + ", z: " + block.getZ() + ") in " + player.getWorld().getName());
			if (trapBlock) {
	...
	public void logLightLevelViolation(final Material mat, final Player player) {
		String lightLogMsg = "[" + getFormattedDate() + "]" + " " + player.getName() + " mined " + Format.getFormattedName(mat, 1) + " below " + fd.getConfig().getString(Config.percentOfLightRequired) + " light";
		try {
			PrintWriter pw = new PrintWriter(new BufferedWriter(new FileWriter(fd.getFileHandler().getLogFile(), true)));
	...
	public void writeToCleanLog(final String matName, final int blockTotal, final String playerName) {
		...
		String message = fd.getConfig().getString(Config.bcMessage).replace("@Prefix@", Prefix.getChatPrefix()).replace("@Player@", playerName).replace("@Number@", blockTotal == 500 ? "over 500" : String.valueOf(blockTotal)).replace("@BlockName@", matName);
		try {
