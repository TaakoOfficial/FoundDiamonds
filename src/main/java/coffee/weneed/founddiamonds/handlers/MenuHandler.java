...
public class MenuHandler {
	...
	public boolean handleSetMenu(FoundDiamonds fd, CommandSender sender, String[] args) {
		if (args.length == 1) {
			...
			if (args[1].equalsIgnoreCase("spellpercent")) {
				...
				sender.sendMessage(Prefix.getChatPrefix() + ChatColor.DARK_RED + " Usage: /fd set " + "spellpercent <percent>");
				return true;
			} else if (args[1].equalsIgnoreCase("itempercent")) {
				sender.sendMessage(Prefix.getChatPrefix() + ChatColor.DARK_RED + " Usage: /fd set " + "itempercent <percent>");
			}
	...
	public void setPercent(FoundDiamonds fd, CommandSender sender, int percent, String configuration) {
		if (percent != 999) {
			...
			sender.sendMessage(Prefix.getChatPrefix() + ChatColor.DARK_RED + " Percentage " + "must be between 0 and 100");
		}
	}
