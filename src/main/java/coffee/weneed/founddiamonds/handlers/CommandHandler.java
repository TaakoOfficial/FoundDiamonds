		...
		if (arg.equalsIgnoreCase("creative")) {
			...
			if (!fd.getFileHandler().getCleanLog().exists()) {
				try {
					...
					if (successful) {
						sender.sendMessage(Prefix.getChatPrefix() + ChatColor.DARK_GREEN + " Cleanlog created.");
					}
			...
			if (args.length == 0) {
				...
				if (arg.equalsIgnoreCase("admin") || arg.equalsIgnoreCase("a")) {
					...
					*/
					...
					if (fd.getPermissions().hasAnyMenuPerm(sender)) {
						sender.sendMessage(Prefix.getChatPrefix() + ChatColor.DARK_RED + " Unrecognized argument '" + ChatColor.WHITE + arg + ChatColor.DARK_RED + "'");
					}
				}
			}
		}
