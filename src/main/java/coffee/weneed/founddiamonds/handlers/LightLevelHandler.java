...
public class LightLevelHandler {
	...
	public void sendLightAdminMessage(final Player player, final Material mat) {
		String lightAdminMessage = ChatColor.YELLOW + player.getName() + ChatColor.GRAY + " is mining " + fd.getMapHandler().getLightLevelBlocks().get(mat) + Format.getFormattedName(mat, 1) + ChatColor.GRAY + " below " + ChatColor.WHITE + fd.getConfig().getString(Config.percentOfLightRequired) + " light";
