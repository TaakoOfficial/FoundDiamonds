...
public class BlockCounter {
	...
	private void findLikeBlocks(BlockFace[] faces, Block passed, Material originalMaterial, Set<Location> blocks, boolean counting) {
		for (BlockFace y : faces) {
			...
			if (var.getType() == originalMaterial && !blocks.contains(var.getLocation()) && isAnnounceable(var.getLocation()) || PluginUtils.isRedstone(var) && PluginUtils.isRedstone(originalMaterial) && isAnnounceable(var.getLocation()) && !blocks.contains(var.getLocation())) {
				if (counting) {
